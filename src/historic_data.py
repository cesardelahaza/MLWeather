...
# Set date
yesterdays = datetime.today() - timedelta(days=1)
print(f"Yesterday was {yesterdays}")
two_weeks_before = yesterdays - timedelta(weeks=2)
...
params = {
	...
	"end_date": yesterdays.strftime('%Y-%m-%d'),
...
for response in responses:
	...
	all_dfs.append(hourly_dataframe)
	print(f"\nHourly data from Latitude {response.Latitude()} and Longitude {response.Longitude()}\n", hourly_dataframe)
	

hourly_dataframe = pd.concat(all_dfs, ignore_index=True)

current_dir = os.path.dirname(__file__)
parent = os.path.abspath(os.path.join(current_dir, os.pardir))
filename = os.path.join(parent, "data/historic_data.csv")

hourly_dataframe.to_csv(filename, index = False)
print(f"\nHistoric data saved to {filename}")